    ...
    <div className="form-container">
      ...
      <section className="form-section">
        ...
        <div className="form-row">
          <label>지원 직종 / 부서</label>
          <input value={data.applyingFor} onChange={e => set('applyingFor', e.target.value)} placeholder="예: 소프트웨어 엔지니어" />
        </div>
        <div className="form-row">
          <label>작성일</label>
      ...
      <section className="form-section">
        ...
        <div className="photo-row">
          ...
          <div className="personal-fields">
            ...
            <div className="form-row">
              <label>성명 (영문)</label>
              <input value={data.personalInfo.nameEn} onChange={e => set('personalInfo.nameEn', e.target.value)} placeholder="HONG GIL DONG" />
            </div>
            <div className="form-row">
              <label>생년월일</label>
        ...
        <div className="form-row">
          <label>주소</label>
          <input value={data.personalInfo.address} onChange={e => set('personalInfo.address', e.target.value)} placeholder="서울특별시 강남구 테헤란로 00길 00" />
        </div>
        <div className="form-row">
          <label>연락처</label>
          <input value={data.personalInfo.phone} onChange={e => set('personalInfo.phone', e.target.value)} placeholder="010-0000-0000" />
        </div>
        <div className="form-row">
          <label>이메일</label>
      ...
      {/* ── 학력사항 ── */}
      <section className="form-section">
        <h2 className="section-title">학력사항</h2>
        {data.education.map((edu) => (
          <div key={edu.id} className="repeatable-item">
            ...
            <div className="form-grid-2">
              <div className="form-row">
                <label>학교명</label>
                <input value={edu.schoolName} onChange={e => updateEducation(edu.id, 'schoolName', e.target.value)} placeholder="OO대학교" />
              </div>
              <div className="form-row">
                <label>전공</label>
                <input value={edu.major} onChange={e => updateEducation(edu.id, 'major', e.target.value)} placeholder="컴퓨터공학과" />
              </div>
              <div className="form-row">
                <label>학위</label>
              ...
              <div className="form-row">
                <label>학점</label>
                <input value={edu.gpa} onChange={e => updateEducation(edu.id, 'gpa', e.target.value)} placeholder="3.8" style={{ width: '80px' }} />
        ...
        <button className="add-btn" onClick={addEducation}>+ 학력 추가</button>
      </section>
        ...
        {data.workExperience.map((work) => (
          <div key={work.id} className="repeatable-item">
            ...
            <div className="form-grid-2">
              <div className="form-row">
                <label>회사명</label>
                <input value={work.companyName} onChange={e => updateWork(work.id, 'companyName', e.target.value)} placeholder="(주)OO회사" />
              </div>
              <div className="form-row">
                <label>부서</label>
                <input value={work.department} onChange={e => updateWork(work.id, 'department', e.target.value)} placeholder="개발팀" />
              </div>
              <div className="form-row">
                <label>직위 / 직책</label>
                <input value={work.position} onChange={e => updateWork(work.id, 'position', e.target.value)} placeholder="선임연구원" />
              </div>
              <div className="form-row">
                <label>입사년월</label>
            ...
            <div className="form-row">
              ...
              <textarea
                ...
                placeholder="주요 업무 내용을 기술해 주세요"
                rows={3}
              />
            </div>
          </div>
        ))}
        <button className="add-btn" onClick={addWork}>+ 경력 추가</button>
      </section>
      ...
      {/* ── 자격증 / 면허 ── */}
      <section className="form-section">
        <h2 className="section-title">자격증 / 면허</h2>
        {data.certifications.map((cert) => (
          <div key={cert.id} className="repeatable-item">
            ...
            <div className="form-grid-2">
              <div className="form-row">
                <label>자격증명</label>
                <input value={cert.name} onChange={e => updateCert(cert.id, 'name', e.target.value)} placeholder="정보처리기사" />
              </div>
              <div className="form-row">
                <label>발급 기관</label>
                <input value={cert.issuer} onChange={e => updateCert(cert.id, 'issuer', e.target.value)} placeholder="한국산업인력공단" />
              </div>
              <div className="form-row">
                <label>취득일</label>
              ...
              <div className="form-row">
                <label>점수 / 급수</label>
                <input value={cert.score} onChange={e => updateCert(cert.id, 'score', e.target.value)} placeholder="1급 / 합격" />
              </div>
            </div>
          </div>
        ))}
        <button className="add-btn" onClick={addCert}>+ 자격증 추가</button>
      </section>

      {/* ── 어학 능력 ── */}
      <section className="form-section">
        <h2 className="section-title">어학 능력</h2>
        {data.languageSkills.map((lang) => (
          <div key={lang.id} className="repeatable-item">
            ...
            <div className="form-grid-2">
              <div className="form-row">
                <label>언어</label>
                <input value={lang.language} onChange={e => updateLang(lang.id, 'language', e.target.value)} placeholder="영어" />
              </div>
              <div className="form-row">
                <label>시험명</label>
                <input value={lang.testName} onChange={e => updateLang(lang.id, 'testName', e.target.value)} placeholder="TOEIC" />
              </div>
              <div className="form-row">
                <label>점수 / 등급</label>
        ...
        <button className="add-btn" onClick={addLang}>+ 어학 추가</button>
      </section>

      {/* ── 수상 / 활동 ── */}
      <section className="form-section">
        <h2 className="section-title">수상 / 대외 활동</h2>
        {data.awards.map((award) => (
          <div key={award.id} className="repeatable-item">
            ...
            <div className="form-grid-2">
              <div className="form-row">
                <label>활동명 / 수상명</label>
                <input value={award.name} onChange={e => updateAward(award.id, 'name', e.target.value)} placeholder="교내 프로그래밍 대회 1위" />
              </div>
              <div className="form-row">
                <label>기관명</label>
                <input value={award.issuer} onChange={e => updateAward(award.id, 'issuer', e.target.value)} placeholder="OO대학교" />
              </div>
              <div className="form-row">
                <label>날짜</label>
              ...
              <div className="form-row">
                <label>내용</label>
                <input value={award.description} onChange={e => updateAward(award.id, 'description', e.target.value)} placeholder="간략한 설명" />
              </div>
            </div>
          </div>
        ))}
        <button className="add-btn" onClick={addAward}>+ 활동 추가</button>
      </section>

      {/* ── 자기소개서 ── */}
      <section className="form-section">
        <h2 className="section-title">자기소개서</h2>
        <p className="section-hint">성장 과정, 성격의 장단점, 지원 동기, 입사 후 포부 등을 기술하세요.</p>
        <textarea
          ...
          rows={12}
        />
        <div className="char-count">{data.selfIntroduction.length}자</div>
      </section>
    </div>
  )
}
